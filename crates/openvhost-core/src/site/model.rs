...
//! The `Site` aggregate and its parse-don't-validate newtypes. Every field
//! that reaches generated config or a filesystem path is charset-checked here,
//! at the boundary (the P0-7 config-injection lesson pushed to ingress).
//!
//! Each newtype wraps a single private field, so `parse` is the only way to
//! build one. This module is a *sibling* of `site::repo` — deliberately not its
//! ancestor — so persistence code cannot name those private fields and smuggle
//! an unvalidated value past `parse`; see the [`site`](super) module docs.
...
macro_rules! newtype_str {
    ($name:ident, $doc:literal) => {
        ...
        impl $name {
            /// The validated inner value as a `&str`.
...
newtype_str!(SiteId, "A site's stable identifier — a v4 UUID string.");
newtype_str!(
    SiteName,
    "A site's short slug name (`[a-z0-9-]`, 1–63 chars, alphanumeric first)."
);
newtype_str!(
    Domain,
    "A site's primary hostname (lowercase dotted labels)."
);
newtype_str!(
    PhpVersion,
...
/// The web server that fronts a site (`nginx` or `apache`).
