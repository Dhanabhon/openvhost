...
/// file now.
///
/// Also seeds `logs/sites` and `logs/services` — the two parent directories
/// under which `site::apply::commit` creates a per-site or per-major
/// directory during an Apply (P1 live-log-viewer design, spec D2) — at mode
/// `0700` (spec D5), via the same [`crate::logs::ensure_log_dir`] `commit()`
/// itself uses, so both call sites enforce "log directories are 0700" from
/// the one shared function rather than two independent copies.
