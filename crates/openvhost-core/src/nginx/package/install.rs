...
//! Installing our own nginx build into OpenVHost's package tree — the third
//! consumer of `openvhost-pkg`, and (like MariaDB) for an artifact we produced
//! ourselves rather than downloaded from upstream (nginx-recipe design, and
...
//! [`crate::mariadb::install_mariadb_package`]'s wiring for the same reason its
//! catalogue gives for the same choice: consistency beats novelty, and a second
//! shape here would be a finding rather than a design.
//!
//! SECURITY: the public entry point takes no arguments that reach the
//! downloader at all. The URL and hash come only from
//! [`crate::nginx::NGINX_PACKAGES`], so there is no argument any caller — IPC,
...
//! nginx has no datadir or credential of its own, and this slice does not give
//! it one; installing a copy of the binary is not running it.
//!
//! **Scope.** Installing a runtime is not running one. Discovering an existing
//! install, choosing it over Homebrew's, starting/stopping it and any
//! Databases- or Web-server-page row are later slices and are deliberately
//! absent here (nginx-recipe design §2 — this slice ends at a verified,
...
/// The result of installing one catalogued nginx build.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NginxPackageInstall {
    ...
    /// the MySQL and MariaDB install paths rather than re-declared: it
    /// describes the ledger, which is package-agnostic, not the package.
    pub ledger: LedgerWrite,
}

/// Install the catalogued nginx build for this host.
///
...
/// `bin/nginx` while the tree is still in staging, renames the tree atomically
/// into `packages/nginx/1.30/<version>/`, swings the per-series `current` link
/// onto it, and records the exact version in `state.db`.
///
/// Takes no version argument: nginx-recipe design D3 pins one stable line
/// only, and adding a second line is a decision with a cost rather than an
...
async fn install_entry(
    ...
    .with_warmup_binary(NGINX_WARMUP_BINARY)?;
    ...
    tracing::info!(
        version = %package.version,
        dir = %package.dir.display(),
        "installed nginx from OpenVHost's own build"
    );

    Ok(NginxPackageInstall {
        package,
        ledger: ledger_write,
    })
}
...
mod tests {
    ...
    /// A `.tar.gz` shaped like our own nginx tarball: one implicit top-level
    /// directory that the extractor strips, `bin/nginx` executable, and a
    /// plain non-executable file alongside it — there is no second binary to
    /// confuse this with, unlike MariaDB's tarball, which also ships
    /// `bin/mariadbd-safe` and `bin/mysqld_safe`.
    fn nginx_shaped_targz(nginx_bin: &str) -> Vec<u8> {
        use flate2::{Compression, write::GzEncoder};
        let gz = GzEncoder::new(Vec::new(), Compression::fast());
        let mut ar = tar::Builder::new(gz);
        let entries: [(&str, &str, u32); 2] = [
            ("nginx-1.30.4/bin/nginx", nginx_bin, 0o755),
            ("nginx-1.30.4/conf/nginx.conf", "# stock sample\n", 0o644),
        ];
    ...
    fn serve_once(body: Vec<u8>) -> String {
        ...
        format!("http://127.0.0.1:{port}/nginx.tar.gz")
    }
    ...
    fn entry_for(url: String, sha256: String) -> NginxPackage {
        NginxPackage {
            major: "1.30",
            version: "1.30.4",
    ...
    impl Fixture {
        ...
        fn version_dir(&self) -> PathBuf {
            self.root.package_dir("nginx", "1.30", "1.30.4")
        }

        fn current_link(&self) -> PathBuf {
            self.root.current_link("nginx", "1.30")
        }
        ...
        async fn ledger_rows(&self) -> usize {
            self.ledger.list("nginx").await.unwrap().len()
        }
    }
    ...
    // binary, and records the version.
    //
    ...
    // `NGINX_PACKAGE_NAME` at "mariadb" moved the install to the wrong tree and
    // failed this group.
    // ------------------------------------------------------------------
    ...
    async fn a_successful_install_lands_at_the_catalogue_version_and_links_current() {
        ...
        let archive = nginx_shaped_targz(&script("exit 0"));
        ...
        assert_eq!(out.package.name, "nginx");
        assert_eq!(out.package.major, "1.30");
        assert_eq!(out.package.version, "1.30.4");
        assert!(
            fx.version_dir().join("bin/nginx").is_file(),
            "the implicit archive root must be stripped so bin/nginx sits at the top"
        );
        assert_eq!(
    ...
    /// nginx's tarball ships exactly one binary under `bin/`, so unlike
    /// MariaDB's twin of this test there is no dangerous wrapper to prove was
    /// NOT warmed instead — only that the one that exists really was.
    #[tokio::test]
    async fn the_install_warms_the_nginx_binary() {
        let fx = Fixture::new().await;
        ...
        assert!(
            warmed.is_file(),
            "bin/nginx was never warmed: the Gatekeeper cost lands on the user's \
             first Start"
        );
    }
    ...
    // MariaDB's identical fixture (see that module's report): truncating a
    // guarded file, and separately rewriting one with IDENTICAL bytes via
    // delete-and-recreate, each turned every test in this group red; the
    // second is the one a content-only check would have missed.
    ...
    // (the same mutation MariaDB's identical test was proven against).
    // ------------------------------------------------------------------
    ...
    async fn the_unpublished_pin_is_refused_before_any_network_or_filesystem_work() {
        ...
        match err {
            CoreError::PackageNotPublished {
                ...
                assert_eq!(url, NGINX_PACKAGES[0].url);
            }
            ref other => panic!("wrong variant: {other:?}"),
        }
        assert!(
            err.to_string().contains("nginx-1.30.4"),
    ...
    // 1.30.4 output.
    //
    // Unlike MariaDB's twin test, this cannot also assert that the output
    // names the hermetic package-tree path: `nginx -v`/`-V` print only a
    // version banner and (for `-V`) the compile-time configure line, never
    // argv[0] or the path used to invoke it — confirmed by reading nginx's
    // own `ngx_show_version_info`. That guarantee instead falls out of HOW
    // the binary is invoked: `Command::new` is given the absolute path inside
    // `fx.version_dir()` directly, never a bare `nginx` resolved through
    // `PATH`, so there is no ambiguity for a string match to rule out (POSIX
    // `exec` with a path containing `/` never consults `PATH`). Adding a
    // string assertion that cannot fail would be exactly the unfailable
    // coverage MariaDB's own report describes removing elsewhere in this
    // pipeline.
    // ------------------------------------------------------------------
    ...
    async fn the_real_artifact_installs_and_runs_from_the_package_tree() {
        ...
        assert_eq!(out.package.dir, fx.version_dir());
        let nginx_bin = fx.version_dir().join("bin/nginx");
        assert!(nginx_bin.is_file(), "{} is missing", nginx_bin.display());

        // `-v`, not `--version`: nginx has no long-option parser at all (the
        // catalogue's own `NGINX_WARMUP_BINARY` doc explains why the
        // pipeline's fixed `--version` warm-up argument is harmless but
        // ignored). `RECIPE_SERVER_VERSION_ARGS=(-v)` in the recipe is the
        // same flag, so this exercises what the artifact contract's own
        // check 2/4 already exercise.
        let output = std::process::Command::new(&nginx_bin)
            .env_clear()
            .arg("-v")
            .output()
            .unwrap_or_else(|e| panic!("exec {}: {e}", nginx_bin.display()));
        // nginx writes its version banner to STDERR, not stdout (confirmed
        // against a real `nginx -v`; `ngx_show_version_info` uses
        // `ngx_log_stderr` because the logging subsystem is not yet
        // initialized this early) — combine both streams rather than assume
        // one, the same way `build/audit.sh`'s own `run_server` does
        // (`>"$log" 2>&1`).
        let combined = format!(
            "{}{}",
        ...
        assert!(
            ...
            "nginx -v exited {}: {combined}",
            output.status,
        );
        assert!(
            combined.contains("1.30.4"),
            "nginx -v printed {combined:?}, which is not the version we installed"
        );
