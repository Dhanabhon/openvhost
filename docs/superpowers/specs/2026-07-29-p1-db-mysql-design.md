...
- staging `<home>/data/mysql/init-<major-dashed>-<uuid>/` — same parent as final, so the finishing `rename` is atomic; no leading dot, so VISIBLE rather than hidden (see the dated correction below for why)

Init sequence (one orchestrated task, `install_php`-style: `run_task` + held `AbortHandle` + Drop guard):

1. `mysqld --no-defaults --initialize-insecure --datadir=<staging> --mysqlx=OFF` — no server, no network, no my.cnf read (every path is explicit argv; `--no-defaults` is deliberate containment; `--mysqlx=OFF` is added for symmetry with step 2 even though this one-shot step starts no server at all — see the dated correction below for the full rationale of both flags). `--initialize` (random expired password in the error log) lost in both designs: the log line is not an API, the password is expired-on-first-use so `ALTER USER` is needed anyway, and a parse failure strands an inaccessible datadir.
2. Spawn temp server: `mysqld --no-defaults --datadir=<staging> --skip-networking --socket=<home>/run/mysql-<major>-init.sock --mysqlx=OFF` — the "insecure window" never has a network listener; the socket lives in the user-only run dir. `--mysqlx=OFF` is LOAD-BEARING here, not decorative — see the dated correction below.
...
**Corrections (2026-07-30, live-gate discoveries against real mysql@8.4.11 — retraction included):** the live gate found three REAL, independent landmines in this sequence, each confirmed with a decisive, single-variable matrix. Two were startup failures; the third — found by the security-auditor gate, not the live gate itself — was a security regression:

1. *A missing `!includedir` target is fatal.* A real mysqld aborts with "Fatal error in defaults handling. Program aborted!" — even for `--validate-config` — when the rendered my.cnf's `!includedir` (spec D5) names a directory that does not exist. Fixed by ensuring `custom_confd` exists wherever my.cnf is produced or used (`write_generated_config`, the one my.cnf write chokepoint; `stack.rs::mysql_spec`, for an already-initialized instance found at startup, in case the directory is deleted later). Unaffected by anything below.
2. *A staging datadir basename that starts with a dot is fatal.* Single-variable matrix on real mysqld 8.4.11: `.stg`, `.a`, `.aaaa`, `.init8`, `.aaaaaaaa`, `.aaa-aaa` all FAIL to restart after `--initialize`; `stg`, `aaa-aaa`, `init-8-4-abc`, and a 24-character all-`a` name all PASS. Hyphens, digits, and length are not factors — only the leading dot. The layout bullet and step 1/2 argv above already show the fix: `init-<major-dashed>-<uuid>`, no leading dot, `major`'s own dot dash-encoded. The directory is consequently visible, not hidden; nothing in the codebase lists its parent directory expecting only major-shaped entries (verified as part of the fix — the one place that does list it, `sweep_stale_staging`, already filters by shape rather than assuming).
3. *Switching to `--no-defaults` silently dropped `mysqlx=OFF` — a security regression, not a startup failure.* `my.cnf` (read via `--defaults-file` in the ORIGINAL design) carried `mysqlx=OFF`; `--no-defaults` reads no config file at all, so the X Plugin reverted to its default ON. Measured directly against real mysql@8.4.11: step 2's exact invocation then binds `/tmp/mysqlx.sock` at mode `srwxrwxrwx` (world read/write, OUTSIDE the 0700 home) AND `*:33060`; `--skip-networking` suppresses the classic protocol's TCP listener only and touches neither. Both are live for the entire window between step 2 starting and step 4 (`ALTER USER`) succeeding — i.e. while `root@localhost` still has the EMPTY password `--initialize-insecure` left it with — so ANY local user, not just a network-adjacent one, could connect as root through that world-writable socket. Fixed by adding `--mysqlx=OFF` explicitly to both steps' argv (step 1 for symmetry only — it starts no server); verified in the same measurement session to bind no socket at all with the flag present.

**Retraction:** two intermediate diagnoses, recorded and then superseded across two earlier edits of this very paragraph, were WRONG and are retracted here for the honest record. The first claimed that combining `--defaults-file=<my.cnf, datadir=final>` with argv `--datadir=<staging>` corrupted InnoDB's undo-tablespace bookkeeping. The second claimed a staging name with more than one dot (regardless of position) was independently fatal. Both were a sloppy test matrix conflating the datadir-mismatch variable, the multi-dot variable, and the one REAL variable — the single leading dot — with each other. Decisive, single-variable re-testing showed the defaults-file+mismatched-argv shape starts and pings fine once the staging basename is dot-free, and that dots in non-leading positions are not a factor at all. A THIRD claim recorded in an earlier edit of this same paragraph — that `--no-defaults` gains exclusion of machine-wide option files (`/etc/my.cnf`, `~/.my.cnf`) — was ALSO wrong: `--defaults-file=<path>` already excludes those on its own; that was never something switching to `--no-defaults` added. `--no-defaults` is KEPT in steps 1-2 above, but for the genuine reason: the rendered my.cnf ends with `!includedir <custom_confd>`, so under `--defaults-file` both steps would read whatever the USER has dropped into that directory — arbitrary user-controlled configuration reaching the init sequence during the empty-password window. `--no-defaults` removes all of it, at the cost of also needing every other setting my.cnf used to carry restated explicitly (see finding 3 above for the one that was missed: `mysqlx=OFF`).
