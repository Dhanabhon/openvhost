...
1. `mysqld --no-defaults --initialize-insecure --datadir=<staging>` — no server, no network, no my.cnf read (every path is explicit argv; `--no-defaults` is deliberate containment — see the dated correction below for why it is NOT a workaround for anything). `--initialize` (random expired password in the error log) lost in both designs: the log line is not an API, the password is expired-on-first-use so `ALTER USER` is needed anyway, and a parse failure strands an inaccessible datadir.
2. Spawn temp server: `mysqld --no-defaults --datadir=<staging> --skip-networking --socket=<home>/run/mysql-<major>-init.sock` — the "insecure window" never has a network listener; the socket lives in the user-only run dir.
...
**Corrections (2026-07-30, live-gate discoveries against real mysql@8.4.11 — retraction included):** the live gate found two REAL, independent landmines in this sequence, each confirmed with a decisive, single-variable matrix:
