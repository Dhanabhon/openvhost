...
- Lock held but socket unreachable → also 69, distinct code `controlChannelUnavailable` ("the app appears to be running but is not accepting control connections — it may still be starting"). The connect result is authoritative; the probe only improves the message.
