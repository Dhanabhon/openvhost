...
The tray lives entirely in Rust: it reads the managed `Arc<Supervisor>` and calls `start`/`stop` directly. No new commands, no JS tray API.

**Correction, 2026-07-31 (security audit finding M2): the claim originally made here — "start-all/stop-all are unreachable from a compromised webview" — was false, and "no `capabilities/default.json` change" was true but did not imply it.** `capabilities/default.json` granted the blanket `core:default`, which — verified against `gen/schemas/acl-manifests.json` — expands to include `core:menu:default` and `core:tray:default`. Those grant the WEBVIEW itself `Menu.setAsAppMenu()`/`MenuItem.setAccelerator()`/`Tray.setMenu()` and siblings. A compromised webview could therefore mint its own native menu item carrying an id `tray::handle_tray_menu_id` treats as authoritative — `openvhost:tray:stop-all`, `openvhost:tray:start-all`, or a literal registered service id — install it, and fire it via `setAccelerator` with no user click, without ever calling a Tauri *command*. "No new commands" was true and is exactly why this was missed: the exposure came from a permission this slice never added or even touched — granted back in the original Phase 0 scaffold (2026-07-21) by the shorthand `core:default`, ten days before this slice — and only made dangerous once this slice built a privileged consumer (`handle_tray_menu_id`) that trusts any incoming menu id. The fix is a capability-file narrowing — `core:default`'s expansion minus `core:menu:default`/`core:tray:default` — not new IPC surface; it is safe because the frontend's only `@tauri-apps` imports are `api/core`, `api/event`, type-only `webview`/`window`, and `plugin-dialog`'s `open`, none of which touch a menu/tray API. See `capabilities/default.json`'s own description for the full before/after set.
