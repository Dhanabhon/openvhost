...
3. Full local gate suite green; **one on-demand CI matrix run green before merge** (first real Windows code — re-enable workflow, `workflow_dispatch`, then re-disable if desired).
   - **Owner decision 2026-07-22:** GitHub Actions is billing-blocked at the account level; the matrix run is DEFERRED (workflow re-disabled per owner instruction). Stand-in evidence for this criterion: full local gate suite + a real `x86_64-pc-windows-msvc` `cargo check`/`clippy` cross-check of `openvhost-proc` (clean). The first matrix run after Actions is restored should backfill this criterion; Windows runtime behavior (ctrl-handler, stop timing) remains unverified until then.
...
- **Windows v0:** `CREATE_NEW_PROCESS_GROUP | CREATE_NO_WINDOW` always (console-window flashes otherwise); graceful stop attempt stays opportunistic (`GenerateConsoleCtrlEvent(CTRL_BREAK, pid)`), **but — correction recorded 2026-07-22 (final review):** `CREATE_NO_WINDOW` gives every child its own hidden console, so CTRL_BREAK never reaches it from us in ANY environment, dev consoles included. **v0 Windows stop is therefore always deadline (5s) + hard kill, everywhere**; real graceful arrives with P0-5 Job Objects / per-service protocol shutdown. Tests use a per-OS stop timeout accordingly. `kill` = `TerminateProcess` in v0; the doc-comment states that from P0-5 it means `TerminateJobObject` on the app-wide Job Object (one job per app, not per child) and must never be simplified back. FFI via `windows-sys` (already in-tree through tokio), not the COM `windows` crate. Job assignment in P0-5 is spawn-then-assign with a documented bounded containment race (correctness/cleanup guarantee, not a security boundary); the opaque `SpawnedChild` is what lets P0-5 switch to raw `CreateProcessW` (suspended→assign→resume needs the thread handle std discards) without a breaking change — verify with a spike at P0-5 start.
