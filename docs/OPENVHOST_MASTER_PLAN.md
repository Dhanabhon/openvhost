# OpenVHost — Development Master Plan & Claude Code Handoff Document

> **Purpose of this document:** This is the single source of truth for the OpenVHost project, written to be consumed by Claude Code (and human contributors). It contains the product definition, confirmed technical decisions, architecture, phased roadmap, and a complete subagent allocation plan with ready-to-use agent definitions.
>
> **Document status:** v1.2 — 2026-07-21 (renamed OpenServ → OpenVHost; brand language finalized)
> **Companion documents:** `docs/OPENVHOST_BRAND_GUIDELINES.md` v1.1 — brand foundation, color/typography tokens, voice & microcopy, trademark summary; the input spec for tauri-frontend-engineer UI work and for `TRADEMARK.md` (OQ#8).
...
**OpenVHost** is an open-source, cross-platform local development environment platform for web developers — a free and open alternative to ServBay / Laragon / XAMPP / MAMP.

| Attribute | Value |
|---|---|
| Product name | **OpenVHost** ✅ — pronounced "open vee-host". Casing O·V·H matches Apache's `<VirtualHost>` directive. **Never abbreviate to "OVH"** (collides with OVHcloud); no short form exists. Renamed 2026-07-21 from working title "OpenServ" after discovering the active collision with OpenServ Labs / openserv.ai (AI-agent platform with SERV token). Web search found no software product using "OpenVHost" as of the rename date. |
| Tagline | **"Open. We host."** ✅ (the "V" reads as "we" — openness + community in three words) |
| README descriptor | **"Your friendly local host"** ✅ (double pun: local dev environment / welcoming host) |
...
| License | **GPL-3.0-or-later** ✅ — owner requires strong copyleft: anyone who distributes OpenVHost or a derivative (modified or commercial) must disclose full source under the same license. AGPL-3.0 was considered and rejected (corporate AGPL bans would shrink the contributor pool; realistic distribution vector for a desktop app is binaries, which GPL covers). See §1.3 |
...
4. **CLI is a first-class citizen:** Everything the GUI can do, `openvhost` can do — with `--json` output for scripting/CI.
5. **Reproducible environments:** A project-level `openvhost.yaml` committed to a repo recreates the same environment for the whole team (our answer to ServBay's `.servbay.config`, but more complete).
6. **Open manifests:** Package definitions live in a public repo (`openvhost/manifests`) so the community can add packages without waiting for an app release.
...
- **App license: GPL-3.0-or-later.** Every distribution of OpenVHost or a derivative work — modified or not, commercial or not — must ship corresponding source under GPL. Commercial *sale* remains permitted (GPL never forbids selling); the obligation is disclosure.
...
- **Inbound contributions: DCO-only 🟡** (`git commit -s`, no CLA). Consequence — and intended feature: copyright stays distributed across all contributors, so **no one, including the project owner, can ever relicense OpenVHost to proprietary**. This makes a Laragon-style closed-source pivot structurally impossible and maximizes community trust. Trade-off: no future dual-licensing/commercial-exception model. (Recorded as OQ#1 with DCO-only as default.)
...
- **Files:** `COPYING` (GPL-3.0 full text) at repo root · SPDX header `GPL-3.0-or-later` in source files · `TRADEMARK.md` asserting the OpenVHost name/logo policy (GPL has no Apache-§6-style trademark clause; trademark law applies independently) · in-app "Open Source Licenses" page listing licenses of every installed package.
...
│  ├─ openvhost-proc   Process supervisor              │
│  ├─ openvhost-pkg    Package manager (dl/verify)     │
│  ├─ openvhost-conf   Config generator (Tera)         │
│  ├─ openvhost-cert   Local CA + leaf certs (P3)      │
│  ├─ openvhost-core   Domain model, state (SQLite)    │
│  └─ openvhost     CLI binary (same crates)        │
└───────────────┬─────────────────────────────────────┘
...
**openvhost-proc (Process Supervisor)** — the heart of the app:
...
**openvhost-pkg (Package Manager):**
- Reads a signed manifest index from the `openvhost/manifests` repo (cached locally).
...
**openvhost-conf (Config Generator):**
...
**openvhost-core:** domain model (Site, ServicePackage, ServiceInstance, Certificate, HostsEntry), SQLite state, event bus. **Must not depend on Tauri** — this keeps it testable and lets `openvhost` reuse it. ✅
...
🟡 Proposed root: `~/.openvhost` on macOS, `%USERPROFILE%\.openvhost` on Windows (CLI-friendly, like `.cargo`/`.nvm`). ❓ Alternative: `C:\OpenVHost` ServBay-style — decide before beta.
...
| P0-3 | `openvhost-proc` v0: spawn/stop/status + log capture | rust-core-engineer | Generic child process managed with state machine |
...
- `openvhost` v1: `start|stop|restart|status|list` with `--json` (mirrors servbayctl verbs; add `reload`, `kill`, `stop-all` for parity).
...
Privileged helper (hosts, 80/443, trust store) with security audit gate · local CA + one-click HTTPS per site (`rcgen`) · CaddyAdapter · dnsmasq/built-in DNS with **wildcard domains** · Mailpit integration · reverse-proxy & CORS UI per site · backup/restore (config+db+ssl+www) · DB major-version upgrade wizard with data migration · `openvhost.yaml` project file (env-as-code) · signed auto-update.
...
|---|---|---|
| `crates/openvhost-core/`, `crates/openvhost-proc/` (common), `crates/openvhost-pkg/`, `apps/cli/` | rust-core-engineer | platform specialists for `#[cfg]` parts |
...
| `templates/**`, `crates/openvhost-conf/` | config-template-engineer | platform specialists for path/socket differences |
| `.github/workflows/**`, `packaging/**`, manifests tooling | ci-release-engineer | security-auditor for signing/verification steps |
| `crates/openvhost-helper/`, `crates/openvhost-cert/`, download-verification code, hosts-file code | rust-core-engineer *implements*, **security-auditor gates** | — |
...
> ✅ Resolved 2026-07-21 — **Name = OpenVHost**, tagline "Open. We host.", descriptor "Your friendly local host" (see §1 table). Web search on rename date found no software product using the name. This resolves the naming half of former OQ#8; registration actions remain below.
...
| 4 | Install root: `~/.openvhost` vs `C:\OpenVHost`-style? | `~/.openvhost` both OSes | Before beta |
...
| 8 | **Human registration actions for the OpenVHost name:** register GitHub org `openvhost`, domains (openvhost.dev / .app preferred), npm + crates.io namespaces, social handles; run trademark searches (DIP Thailand, USPTO, WIPO); publish `TRADEMARK.md` | Register ASAP — squatting risk rises the moment the name is public | Before public announcement |

## 8. Reference Notes (from ServBay docs analysis, 2026-07)

Patterns deliberately adopted: per-major shared config dirs · `current` version link · data dir per DB major version · pause-by-comment for hosts entries · generated-config warnings (we improve with in-file banners + diff preview) · `servbayctl` verb set (`start|stop|reload|restart|kill|status|stop-all`) mirrored in `openvhost` with added `--json`.
